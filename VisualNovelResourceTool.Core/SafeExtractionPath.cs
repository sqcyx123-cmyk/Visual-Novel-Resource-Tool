...
public sealed class SafeExtractionPath
{
    ...
    public void WriteReport(IEnumerable<string> failures)
    {
        ...
        File.WriteAllLines(Path.Combine(_root, "_提取记录.txt"), lines, new UTF8Encoding(true));
    }
    ...
    private static string Sanitize(string value)
    {
        ...
        if (Reserved.Contains(Path.GetFileNameWithoutExtension(result))) result = "_" + result;
        if (result.Length > 180)
        {
            var ext = Path.GetExtension(result); var hash = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(value)))[..8].ToLowerInvariant();
