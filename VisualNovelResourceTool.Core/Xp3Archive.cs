...
public sealed class Xp3Archive
{
    ...
    public static Xp3Archive Open(string path, CancellationToken cancellationToken = default)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!visited.Add(file.Position)) throw new InvalidDataException("XP3 索引形成循环。");
            var flag = ReadByte(file); more = (flag & 0x80) != 0; var compressed = (flag & 7) == 1;
            if ((flag & 7) > 1) throw new InvalidDataException("XP3 索引压缩方式不受支持。");
            var stored = checked((long)ReadUInt64(file)); var original = compressed ? checked((long)ReadUInt64(file)) : stored;
            if (stored < 0 || original < 0 || stored > file.Length - file.Position || original > 512L * 1024 * 1024 - index.Length) throw new InvalidDataException("XP3 索引大小异常。");
            var nextBlock = file.Position + stored;
            using var slice = new LimitedStream(file, stored);
            if (compressed) { using var z = new ZLibStream(slice, CompressionMode.Decompress); BoundedCopy.Copy(z,index,original,cancellationToken); }
            else BoundedCopy.Copy(slice,index,original,cancellationToken);
            file.Position = nextBlock;
            if (more)
            {
                // The continuation is an offset to the next index, not an inline block.
                var nextOffset = checked((long)ReadUInt64(file));
                if (nextOffset < Signature.Length + 8 || nextOffset >= file.Length) throw new InvalidDataException("XP3 后续索引偏移无效。");
        ...
        while (index.Position < index.Length)
        {
            ...
            while (index.Position < end)
            {
                ...
                if (subTag == "info") { if(subSize<22)throw new InvalidDataException("XP3 info 块过短。");ReadUInt32(index);length = checked((long)ReadUInt64(index)); ReadUInt64(index); var chars = ReadUInt16(index);if(chars*2>subEnd-index.Position)throw new InvalidDataException("XP3 文件名越界。"); name = Encoding.Unicode.GetString(Read(index, chars * 2)); }
                else if (subTag == "segm") { if(subSize%28!=0)throw new InvalidDataException("XP3 数据段索引大小无效。");while (index.Position + 28 <= subEnd) { cancellationToken.ThrowIfCancellationRequested();var flags = ReadUInt32(index); var offset = checked((long)ReadUInt64(index)); var original = checked((long)ReadUInt64(index)); var stored = checked((long)ReadUInt64(index)); if ((flags&7)>1 || offset < 0 || stored < 0 || offset > file.Length || stored > file.Length - offset || ((flags&7)==0&&original!=stored)) throw new InvalidDataException("XP3 数据段越界或压缩信息无效。"); segments.Add(new((flags & 7) == 1, offset, original, stored)); } }
                index.Position = subEnd;
            }
            index.Position = end; if (!string.IsNullOrWhiteSpace(name) && segments.Count > 0) { if(segments.Sum(s=>s.OriginalSize)!=length)throw new InvalidDataException("XP3 数据段总长与文件长度不一致。");entries.Add(new(name.Replace('\\','/'), length, segments)); }
        }
    ...
    public async Task<ExtractionSummary> ExtractAsync(IEnumerable<Xp3Entry> selectedEntries, string outputDirectory, bool overwrite, IProgress<ExtractionProgress>? progress = null, CancellationToken cancellationToken = default, bool addChineseDirectoryLabels = false)
    {
        ...
        for (var i = 0; i < items.Length; i++)
        {
            ...
            try { await using var output = new FileStream(temp, FileMode.CreateNew, FileAccess.Write, FileShare.None, 1024 * 1024, FileOptions.Asynchronous); await CopyEntryAsync(source, entry, output, cancellationToken); await output.FlushAsync(cancellationToken); await output.DisposeAsync(); File.Move(temp, target, overwrite); written += entry.Length; extracted++; }
    ...
    {
        foreach (var segment in entry.Segments) { token.ThrowIfCancellationRequested();source.Position = segment.Offset; using var limited = new LimitedStream(source, segment.StoredSize); if (segment.Compressed) { using var z = new ZLibStream(limited, CompressionMode.Decompress, true); await BoundedCopy.CopyAsync(z,output,segment.OriginalSize,token); } else await BoundedCopy.CopyAsync(limited,output,segment.OriginalSize,token); }
